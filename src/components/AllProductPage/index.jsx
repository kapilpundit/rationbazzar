...
export default function AllProductPage({ response, sellerInfo }) {
  ...
  const products = resProducts?.length
...
useEffect(() => {
  ...
  if (data) {
    const { products: { data: productData, next_page_url }, categories, activeVariants, brands } = data;
    setProducts(productData);
    setNxtPage(next_page_url);
    setCategoriesFilter(categories?.map((item) => ({ ...item, selected: false })) || []);
    setVariantsFilter(
      activeVariants?.map((variant) => ({
        ...variant,
        active_variant_items: variant.active_variant_items?.map((variant_item) => ({
          ...variant_item,
          selected: false,
        })) || [],
      })) || []
    );
    setBrands(brands?.map((item) => ({ ...item, selected: false })) || []);
    setVolume({
      min: productData ? Math.min(...productData.map((item) => parseInt(item.price))) : 0,
      max: productData ? Math.max(...productData.map((item) => parseInt(item.price))) : 0,
    });
  }
}, [response.data]);
...
//   const { products: { data: productData } } = response.data;

//   const min = Math.min(...productData.map((item) => parseInt(item.price)));
//   const max = Math.max(...productData.map((item) => parseInt(item.price)));
...
//   if (check) {
//     const brandsQuery = selectedBrandsFilterItem.length > 0
//       ? selectedBrandsFilterItem.map((value) => `brands[]=${value}`)
//       : [];
//     const brandString = brandsQuery.length > 0 ? brandsQuery.join("&") + "&" : "";

//     const categoryQuery = selectedCategoryFilterItem.length > 0
//       ? selectedCategoryFilterItem.map((value) => `categories[]=${value}`)
//       : [];
//     const categoryString = categoryQuery.length > 0 ? categoryQuery.join("&") + "&" : "";

//     const variantQuery = selectedVarientFilterItem.length > 0
//       ? selectedVarientFilterItem.map((value) => `variantItems[]=${value}`)
//       : [];
//     const variantString = variantQuery.length > 0 ? variantQuery.join("&") + "&" : "";

//     axios
//       .get(`${process.env.NEXT_PUBLIC_BASE_URL}api/search-product?${brandString}${categoryString}${variantString}min_price=${volume.min}&max_price=${volume.max}${sellerInfo ? `&shop_name=${sellerInfo.seller.slug}` : ""}`)
//       .then((res) => {
//         const { data: { products: { data, next_page_url } } } = res;

//         setProducts(data.length > 0 ? data : productData);
//         setNxtPage(next_page_url);
//       })
//       .catch((err) => {
//         console.log(err);
...
//     setProducts(productData);
//   }
// }, [selectedVarientFilterItem, selectedCategoryFilterItem, selectedBrandsFilterItem, volume, response.data]);
